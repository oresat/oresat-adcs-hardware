...
The OreSat Mag Card handles three things for the open source [OreSat CubeSat system](https://www.oresat.org/):

- Reads the four magnetometers located on the end caps of the satellite and broadcasts the data on the CAN bus.
- Reads the 6 DOF inertial measurement unit (IMU) and broadcasts the data on the CAN bus (mostly just the gyroscopes).
- Runs the three magnetorquers based on commands from the C3 via the CAN bus.

## Documentation Links

- Mag card hardware Git repository: <https://github.com/oresat/oresat-adcs-hardware>
- **TODO:** Mag card Design Notes + Design Review Notes
